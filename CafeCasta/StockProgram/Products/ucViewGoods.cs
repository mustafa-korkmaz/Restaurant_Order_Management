...
{
    public partial class ucViewGoods : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public ucViewGoods(int good_id)
        {
        ...
        private void SetShoeSizeButtons()
        {
            ...
            string sql = "SELECT * FROM product_size_limits  WHERE  product_id=" + this.goods_id;
            int minSize = 0;
            int maxSize = 0;
            try
            {
        ...
        {
            DBObjects.MySqlDbHelper db = new MySqlDbHelper(StaticObjects.MySqlConn);
            string sql = "SELECT * ,sum(product_count) as total_product_count FROM v_stocks  WHERE  product_id=" + this.goods_id+"  group by product_size";
            try
            {
                DataTable dt = db.GetDataTable(sql);
                if (dt.Rows.Count <= 0)
        ...
        private void FillGrid()
        {
         ...
         //   string sql = "select pv.*,(pv.product_buy_amount-pv.product_return_count) as product_net_buy_amount,((pv.product_buy_amount-pv.product_return_count)*pv.buy_price) as total_buy_price,pd.product_price from v_product_view pv inner join product_details pd on(pv.product_id=pd.product_id) where pv.product_id=" + this.product_id;
            string sql = "select gv.*,(cost+kdv-return_price) as total_cost,((cost+kdv-return_price)/(total_buy_amount-total_return_amount)) as avg_buy_price from v_goods_view gv where gv.goods_id=" + this.goods_id;
            try
            {
